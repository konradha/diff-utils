    ...
    def backward(ctx, grad_x):
        ...
        with torch.enable_grad():
            for m in range(M):
                ...
                dD_dx = torch.autograd.grad(delta, x_m, grad_outputs=grad_ones, create_graph=False, retain_graph=True)[0]
                if dD_dx.abs() < 1e-30:
                    continue
                dD_dtheta = torch.autograd.grad(delta, theta_detached, grad_outputs=grad_ones, create_graph=False, allow_unused=True)
